...
if(!is_error($site)) {
	...
	if (!empty($_GPC['state']) && strexists($_GPC['state'], 'we7sid-') && (empty($_W['openid']) || empty($_SESSION['openid']))) {
	    if(!empty($_GPC['api_openid'])){
	        $_W['openid'] = $_SESSION['openid'] = $_GPC['api_openid'];
	        $fans_uid = pdo_fetchcolumn("select uid from ".tablename("mc_mapping_fans")." where openid = '".$_W['openid']."' ");
            $_W['uid']    = empty($_SESSION['uid']) ? $fans_uid : $_SESSION['uid'];
            $_W['member'] = pdo_fetch("select * from ".tablename("mc_members")." where uid = ".intval($_W['uid']));
	    }else{
