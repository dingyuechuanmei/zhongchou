...
if ($do == 'openid') {
	...
	if (!empty($oauth) && !is_error($oauth)) {
		...
		$account_api->result(0, '', array('sessionid' => $_W['session_id'],'unionid'=>$oauth['openid']));
	} else {
	...
	if(!empty($userinfo['openId'])){
	    $user_info = pdo_fetch("SELECT id,login_nums FROM ".tablename('test_center_user')." WHERE uniacid =:uniacid AND openid =:openid limit 1",array(':uniacid'=>$_W['uniacid'],':openid'=>$userinfo['openId']));
	    $data = array(
	        'uniacid'=>$_W['uniacid'],
	        'unionid'=>$userinfo['unionId'],
	        'openid'=>$userinfo['openId'],
	    ...
	    if(!$user_info){
	        pdo_insert('test_center_user',$data);
	    }else{
	        $data['login_nums'] = intval($user_info['login_nums'])+1;
	        pdo_update('test_center_user',$data,array('id'=>$user_info['id']));
	    }
	}
