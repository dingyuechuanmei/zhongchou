...
if(!empty($setting['payment']['transfar']['switch'])) {
	$dos[] = 'transfar';
}
$do = $_GPC['do'];
...
if(!empty($type)) {
	...
	if($type == 'transfar'){
	    if(!empty($plid)) {
	        ...
	        $tag['openid'] = $_W['openid'];
	        pdo_update('core_paylog', array('openid' => $_W['member']['uid'], 'tag' => iserializer($tag)), array('plid' => $log['plid']));
	    }
	     
	    $ret = transfar_build($ps, $setting['payment']['transfar']);
	    if($ret['url']) {
