	...
	public function notice() 
	{
		...
		if (isset($data['openid3'])) 
		{
			if (!(empty($data['openid3']))) 
			{
				...
				if($_W['uniacid'] == 61){
					$salers3 = pdo_fetchall('select fanid id,nickname,openid,uid,tag from ' . tablename('mc_mapping_fans') . ' where openid in (' . implode(',', $openids3) . ') and uniacid=' . $_W['uniacid']);
					foreach ($salers3 as $key => &$value) {
						$value['avatar'] = pdo_fetchcolumn("select avatar from ".tablename("mc_members")." where uniacid=:uniacid and uid =:uid ",array(':uniacid'=>$_W['uniacid'],':uid'=>$value['uid']));
						if(empty($value['avatar'])){
							$tag = unserialize(base64_decode($value['tag']));
							$value['avatar'] = $tag['headimgurl'];
