...
class Order_EweiShopV2Model {
    public function fullback($orderid) {
        global $_W;
        $uniacid = $_W['uniacid'];
        $order_goods = pdo_fetchall('select o.openid,og.optionid,og.goodsid,og.price,og.total from ' . tablename('ewei_shop_order_goods') . ' as og' .' left join ' . tablename('ewei_shop_order') . ' as o on og.orderid = o.id' .' where og.uniacid = ' . $uniacid . ' and og.orderid = ' . $orderid . ' ');
        $member = m('member')->getMember($order_goods['openid']);
        ...
        foreach ($order_goods as $value) {
            ...
            $fullbackgoods = pdo_fetch('SELECT id,minallfullbackallprice,maxallfullbackallprice,minallfullbackallratio,maxallfullbackallratio,`day`,' . '                          fullbackprice,fullbackratio,status,hasoption,marketprice,`type`,startday' . ' FROM ' . tablename('ewei_shop_fullback_goods') . ' WHERE uniacid = ' . $uniacid . ' and goodsid = ' . $value['goodsid'] . ' limit 1');
            if (!(empty($fullbackgoods)) && $goods['hasoption'] && (0 < $value['optionid'])) {
                $option = pdo_fetch('select id,title,allfullbackprice,allfullbackratio,fullbackprice,fullbackratio,`day` from ' . tablename('ewei_shop_goods_option') . ' ' . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid and isfullback = 1 limit 1', array(':uniacid' => $uniacid, ':goodsid' => $value['goodsid'], ':id' => $value['optionid']));
                if (!(empty($option))) {
