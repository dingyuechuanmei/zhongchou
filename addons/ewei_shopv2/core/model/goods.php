...
class Goods_EweiShopV2Model 
{
	...
	public function getList($args = array()) 
	{
		...
		//增加商户手机端后台关闭商城以后不显示该商户商品，搜索不到该商户商品
		$condition = ' and `uniacid` = :uniacid AND `deleted` = 0 AND status=1 AND id NOT IN (SELECT id FROM '.tablename('ewei_shop_goods').' WHERE uniacid = :uniacid AND merchid IN (SELECT id FROM '.tablename('ewei_shop_merch_user').' WHERE status = 0 AND uniacid = :uniacid))';
		...
		if (!($random)) 
		{
			$sql = 'SELECT id,title,thumb,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,hasoption,unit' . "\r\n" . '            FROM ' . tablename('ewei_shop_goods') . ' where 1 ' . $condition . ' ORDER BY ' . $order . ' ' . $orderby . ' LIMIT ' . (($page - 1) * $pagesize) . ',' . $pagesize;
	...
	public function getTotals($merch = 0) 
	{
        ...
        return array(
		    'sale' => pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_goods') . ' where status > 0 and checked=0 and deleted=0 and total>0 and uniacid=:uniacid'.$condition, array(
		        ':uniacid' => $_W['uniacid']
		    )),
		    'out' => pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_goods') . ' where status > 0 and deleted=0 and total=0 and uniacid=:uniacid'.$condition, array(
		        ':uniacid' => $_W['uniacid']
		    )),
		    'stock' => pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_goods') . ' where (status=0 or checked=1) and deleted=0 and uniacid=:uniacid'.$condition, array(
		        ':uniacid' => $_W['uniacid']
		    )),
		    'cycle' => pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_goods') . ' where deleted=1 and uniacid=:uniacid'.$condition, array(
		        ':uniacid' => $_W['uniacid']
		    ))
		);
	}
