...
class Finance_EweiShopV2Model 
{
	public function pay($openid = '', $paytype = 0, $money = 0, $trade_no = '', $desc = '', $return = true) 
	{
		...
		$pars = array();
		$pars['mch_appid'] = $payment['sub_appid'];     //1314914301
     ...
     * @return array|bool
     */
	public function myPay($openid,$money,$trade_no = '', $desc = ''){
        $data = m('common')->getSysset('app');
        $sec = m('common')->getSec();
        $sec = iunserializer($sec['sec']);
        $url = 'https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers';
        $pars = array();
        $pars['mch_appid'] = $data['appid'];     //1314914301
        $pars['mchid'] = $sec['wxapp']['mchid'];
        $pars['nonce_str'] = random(32);
        $pars['partner_trade_no'] = ((empty($trade_no) ? time() . random(4, true) : $trade_no));
        $pars['openid'] = $openid;
        $pars['check_name'] = 'NO_CHECK';
        ...
        $xml = array2xml($pars);
        $extras = array();
        $certfile = IA_ROOT . '/addons/ewei_shopv2/cert/apiclient_cert.pem';
        $keyfile = IA_ROOT . '/addons/ewei_shopv2/cert/apiclient_key.pem';
        $rootfile = IA_ROOT . '/addons/ewei_shopv2/cert/rootca.pem';
        $extras['CURLOPT_SSLCERT'] = $certfile;
        $extras['CURLOPT_SSLKEY'] = $keyfile;
        $extras['CURLOPT_CAINFO'] = $rootfile;
