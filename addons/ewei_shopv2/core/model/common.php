...
class Common_EweiShopV2Model 
{
     ...
     * @return string 				返回状态
     */
    function send_msg($to_mobile,$msg){
        $url = "http://cf.51welink.com/submitdata/service.asmx/g_Submit";
        $config = m('common')->getSysset('lexin');
        $data = "sname=".$config['lexin_username']."&spwd=".$config['lexin_password']."&scorpid=".$config['lexin_notice']."&sprdid=".$config['lexin_verify']."&sdst={$to_mobile}&smsg={$msg}【额尔古纳微客】";
        $curl = curl_init();
