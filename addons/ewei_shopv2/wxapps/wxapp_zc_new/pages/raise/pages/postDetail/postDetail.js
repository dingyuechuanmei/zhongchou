...
Page({
  ...
  agreecomment: function (e) {
    var review = me.data.review_list
    for (var i=0;i<review.length;i++) {
      if (review[i].id == e.currentTarget.dataset.review_id) {
        review[i].is_prase = review[i].is_prase.indexOf('agree-on') > 1 ? '../../resource/image/agree.png' : '../../resource/image/agree-on.png'
      }
    }
    me.setData({
      'review_list': review
  ...
  back_report: function (e) {
    var review_list = me.data.review_list
    for (var i = 0, l = review_list.length; i < l; i++) {
      if (review_list[i].id == e.currentTarget.dataset.idx) {
        review_list[i].pop = !e.currentTarget.dataset.pop
      } else {
        review_list[i].pop = false
      }
    }
    me.setData({
      review_list: review_list
