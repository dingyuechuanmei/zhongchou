...
Page({
  ...
  applyWithdrawals:function(e){
    var openid = this.data.openids
    var keti = this.data.keti
    var raise_service = this.data.raise_service
        wx.navigateTo({
          url: '/pages/member/withdraw/zongcou?openid=' + openid + '&keti=' + keti + '&raise_service=' + raise_service,
        })
  },
  // onload部分
    ...
    switch (idx) {
      ...
      case 4:
        ...
        a.post('raise.center', {}, function (json) {
           console.log(json)
          self.setData({
            openids:json.merch.openid,
            zonge: json.refer_money,  //总的众筹金额
            keti: json.postal_money,//可提现金额
            raise_service: json.raise_service,//可提现金额
