...
class EweiShopWechatPay 
{
		...
		if (!(empty($log)) && ($log['status'] == '0') && ($log['fee'] == $this->total_fee)) 
		{
			...
			if (!(is_error($site))) 
			{
				...
				if (method_exists($site, $method)) 
				{
                    ...
                    //发送模板消息
                    $order = pdo_get('ewei_shop_order', array('ordersn' => $log['tid'], 'uniacid' => $log['uniacid']), ['id','openid','prepayid','parentid','merchid']);
                    if (!empty($order['prepayid'])) {
                        $openid = substr($order['openid'],7);
                        $template_id = 'PmcfA8VYxLBBBXu4qi0ph6KJ3tlYQbsDbS4cNsULU_0';
                        $prepay_id = $order['prepayid'];
                        $msg = array(
                            'keyword1' => ['value' => $log['tid']],
                            'keyword2' => ['value' => $log['fee'].'元'],
                            'keyword3' => ['value' => date('Y-m-d H:i:s',time())],
                            'keyword4' => ['value' => '微客订单'],
                        );
                        m('qrcode')->myAppSendTplMessage($openid,$template_id,$prepay_id,$msg);
                    }
	...
	public function starter_help() 
	{
		...
		if (!(empty($log)) && ($log['status'] == '0') && ($log['fee'] == $this->total_fee)) 
		{
			...
			$record['tag'] = iserializer($log['tag']);
			pdo_update('core_paylog', $record, array('plid' => $log['plid']));
			pdo_update('ewei_shop_raise_starter_help', array('status' => 1,'transaction_id'=>$this->get['transaction_id']), array('ordersn' => $log['tid'], 'uniacid' => $log['uniacid']));
			return;
		}
		else 
		{
