...
class Setting_EweiShopV2Page extends PluginWebPage 
{
	...
	public function setlottery() 
	{
		global $_W;
		global $_GPC;
		$set = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_luckdraw_default') . ' WHERE uniacid =:uniacid LIMIT 1', array(':uniacid' => $_W['uniacid']));
		if (!(empty($set)))
		{
			$detail = unserialize($set['data']);
		}
		if ($_W['ispost'])
		{
			$data = ((is_array($_GPC['data']) ? $_GPC['data'] : array()));
			$detail['info'] = serialize($_GPC['info']);
			$data = array('data' => serialize($detail), 'isopen' => intval($_GPC['isopen']),'addtime' => time());
			$count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_luckdraw_default') . ' WHERE uniacid =:uniacid', array(':uniacid' => $_W['uniacid']));
			if ($count) 
			{
				pdo_update('ewei_shop_luckdraw_default', $data, array('uniacid' => $_W['uniacid']));
			}
			else 
			{
				$data['uniacid'] = $_W['uniacid'];
				pdo_insert('ewei_shop_luckdraw_default', $data);
			}
			show_json(1, array('url' => webUrl('luckdraw/setting/setlottery')));
		}
		include $this->template();
	}
}
