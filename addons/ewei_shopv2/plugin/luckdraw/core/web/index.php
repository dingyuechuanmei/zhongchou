...
class Index_EweiShopV2Page extends PluginWebPage 
{
	public function main() 
	{
		...
		if (!(empty($_GPC['keyword']))) 
		{
			...
			$condition .= ' AND `luckdraw_title` LIKE :title';
			$params[':title'] = '%' . trim($_GPC['keyword']) . '%';
		}
		$list = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_luckdraw') . ' WHERE 1 ' . $condition . ' ORDER BY addtime desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_luckdraw') . ' where 1 ' . $condition . ' ', $params);
		$pager = pagination($total, $pindex, $psize);
		foreach ($list as $key => $value ) 
		{
			$member_total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_luckdraw_log') . ' where luckdraw_id=:luckdraw_id ', array(':luckdraw_id' => $value['luckdraw_id']));
			$list[$key]['viewcount'] = $member_total ? $member_total : 0;
		}
	...
	protected function post() 
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$type = intval($_GPC['luckdraw_type']);
		$thumb = EWEI_SHOPV2_STATIC.'images/nopic.jpg';
		if ($id) 
		{
			$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_luckdraw') . ' WHERE luckdraw_id =:luckdraw_id and uniacid=:uniacid limit 1', array(':luckdraw_id' => $id, ':uniacid' => $_W['uniacid']));
			$type = intval($item['luckdraw_type']);
			$reward = unserialize($item['luckdraw_data']);
		}
		...
		if ($_W['ispost']) 
		{
			...
			if ((intval($_GPC['luckdraw_type']) == 3) && ($itemCount != 8) && !(empty($itemCount)))
			{
				show_json(0, '九宫格奖项必须为8个');
			}
			else if ((intval($_GPC['luckdraw_type']) == 1) && ($itemCount < 5) && !(empty($itemCount)))
			{
				show_json(0, '大转盘奖项不得少于5个');
			}
			$data = array();
			$data['uniacid'] = $_W['uniacid'];
			$data['luckdraw_title'] = $_GPC['luckdraw_title'];
			$data['luckdraw_consume'] = $_GPC['luckdraw_consume'];
			$data['luckdraw_icon'] = trim($_GPC['luckdraw_icon']);
			$data['luckdraw_banner'] = trim($_GPC['luckdraw_banner']);
			$data['luckdraw_days'] = intval($_GPC['luckdraw_days']) * 24 * 3600;
			$data['luckdraw_cannot'] = trim($_GPC['luckdraw_cannot']);
			$data['start_time'] = strtotime($_GPC['time']['start']);
			$data['end_time'] = strtotime($_GPC['time']['end']);
			$data['is_default'] = $_GPC['is_default'];
			$data['luckdraw_type'] = intval($_GPC['luckdraw_type']);
			$data['addtime'] = time();
			if ($_GPC['task_type'] == 1) 
			{
			...
			$data['luckdraw_data'] = serialize($reward);
			if ($id)
			{
				$res = pdo_update('ewei_shop_luckdraw', $data, array('luckdraw_id' => $id, 'uniacid' => $_W['uniacid']));
				if ($res) 
				{
				...
				show_json(1, array('url' => webUrl('luckdraw')));
			}
			else 
			{
				$res = pdo_insert('ewei_shop_luckdraw', $data);
				$id = pdo_insertid();
				if ($res) 
				{
				...
				show_json(1, array('url' => webUrl('luckdraw/edit', array('id' => $id, 'luckdraw_type' => $data['luckdraw_type']))));
			}
		}
		include $this->template();
	}
	public function delete() 
	{
		...
		$posters = pdo_fetchall('SELECT luckdraw_id,luckdraw_title FROM ' . tablename('ewei_shop_luckdraw') . ' WHERE luckdraw_id in ( ' . $id . ' ) and uniacid=' . $_W['uniacid']);
		foreach ($posters as $poster )
		{
			pdo_update('ewei_shop_luckdraw', array('is_delete' => 1), array('luckdraw_id' => $poster['luckdraw_id'], 'uniacid' => $_W['uniacid']));
			plog('lottery.delete', '删除抽奖 ID: ' . $id . ' 海报名称: ' . $poster['luckdraw_title']);
		}
		show_json(1, array('url' => webUrl('luckdraw')));
	}
