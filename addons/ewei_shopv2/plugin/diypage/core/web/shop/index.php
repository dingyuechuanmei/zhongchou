...
class Index_EweiShopV2Page extends PluginWebPage 
{
	...
	public function page() 
	{
		...
		$home_list = $this->model->getPageList('allpage', ' and type=2 '.$condition);
		$home_list = $home_list['list'];
		$member_list = $this->model->getPageList('allpage', ' and type=3 '.$condition);
		$member_list = $member_list['list'];
		$commission_list = $this->model->getPageList('allpage', ' and type=4 '.$condition);
		$commission_list = $commission_list['list'];
		$detail_list = $this->model->getPageList('allpage', ' and type=5 '.$condition);
		$detail_list = $detail_list['list'];
		$creditshop_list = $this->model->getPageList('allpage', ' and type=6 '.$condition);
		$creditshop_list = $creditshop_list['list'];
		$seckill_list = $this->model->getPageList('allpage', ' and type=7 '.$condition);
		$seckill_list = $seckill_list['list'];
		$exchange_list = $this->model->getPageList('allpage', ' and type=8 '.$condition);
	...
	public function location() 
	{
		...
		global $_GPC;
		$diypagedata = m('common')->getPluginset('diypage');
		$diylayer = $diypagedata['location'];
		if ($_W['ispost']) 
		{
			$data = $_GPC['data'];
			if (!(empty($data))) 
			{
				$diypagedata['location'] = $data;
				m('common')->updatePluginset(array('diypage' => $diypagedata));
				plog('diypage.shop.location.main', '编辑自定义地址定位按钮');
				show_json(1);
			}
			show_json(0, '数据错误，请刷新页面重试！');
		}
