...
class DiypageModel extends PluginModel 
{
										...
										foreach ($goodsids as $goodsid ) 
										{
											foreach ($goods as $index => $good ) 
											{
												if ($good['id'] == $goodsid) 
												{
													...
													$childid = 'C' . $childid;
													$item['data'][$childid] = array('thumb' => tomedia($good['thumb']), 'title' => $good['title'], 'price' => $good['minprice'], 'gid' => $good['id'], 'total' => $good['total'], 'bargain' => $good['bargain'], 'productprice' => $good['productprice'], 'credit' => $good['credit'], 'ctype' => $good['type'], 'gtype' => $good['goodstype'], 'sales' => $good['sales'] + intval($good['salesreal']));
												}
											}
										}
									}
	...
	public function saveImg($str) 
	{
		...
		$str = preg_replace_callback('/"imgurl"\\:"([^\\\'" ]+)"/', function($matches)
        {
            ...
            if(!(strpos($preg,'http') !== false)){
                if((strpos($preg,'../addons/ewei_shopv2/') !== false)){
                    $preg = str_replace("../addons/ewei_shopv2/",EWEI_SHOPV2_URL,$preg);
                }else{
                    $preg = str_replace("images/",SITE_URL."attachment/images/",$preg);
                }
            }
        ...
        $str = preg_replace_callback('/"iconurl"\\:"([^\\\'" ]+)"/', function($matches)
        {
            $preg = ((!(empty($matches[1])) ? istripslashes($matches[1]) : ''));
            if (empty($preg))
            {
            ...
            $newUrl = save_media($preg);
            return '"iconurl":"' . $newUrl . '"';
        }, $str);

        $str = preg_replace_callback('/"icon"\\:"([^\\\'" ]+)"/', function($matches)
        {
            $preg = ((!(empty($matches[1])) ? istripslashes($matches[1]) : ''));
            if (empty($preg))
            {
        ...
        $str = preg_replace_callback('/"thumb"\\:"([^\\\'" ]+)"/', function($matches)
        {
            $preg = ((!(empty($matches[1])) ? istripslashes($matches[1]) : ''));
            if (empty($preg))
            {
        ...
        $str = preg_replace_callback('/"audiourl"\\:"([^\\\'" ]+)"/', function($matches)
        {
            $preg = ((!(empty($matches[1])) ? istripslashes($matches[1]) : ''));
            if (empty($preg))
            {
            ...
            if(!(strpos($preg,'http') !== false)){
                if((strpos($preg,'../addons/ewei_shopv2/') !== false)){
                    ...
                    $preg = str_replace("audios/",SITE_URL."attachment/audios/",$preg);
                }
            }
            $newUrl = save_media($preg);
            return '"audiourl":"' . $newUrl . '"';
        }, $str);

        $str = preg_replace_callback('/"headurl"\\:"([^\\\'" ]+)"/', function($matches)
        {
            $preg = ((!(empty($matches[1])) ? istripslashes($matches[1]) : ''));
            if (empty($preg))
            {
        ...
        $str = preg_replace_callback('/"content"\\:"([^\\\'" ]+)"/', function($matches)
        {
            ...
            $preg = mload()->model('common')->html_images($preg);
