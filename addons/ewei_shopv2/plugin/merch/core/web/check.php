...
class Check_EweiShopV2Page extends PluginWebPage 
{
	...
	public function test()
    {
        global $_W;
        global $_GPC;
        $openid = 'ojIoH0dhaUzFmQSzG03Jd2X3MdEo';
        $payprice = 10;
        $item['applyno'] = 'ME20180917115354030474'.rand(100000,999999);
        $data = m('finance')->myPay($openid, $payprice, $item['applyno'], '商户提现申请打款');
        var_dump($data);
    }
	...
	public function merchpay() 
	{
		...
		if (empty($handpay) && empty($item['applytype'])) 
		{
            ...
            $merch_user['payopenid'] = substr($merch_user['payopenid'],7);
			$result = m('finance')->myPay($merch_user['payopenid'], $payprice, $item['applyno'], '商户提现申请打款');
			if (is_error($result)) 
			{
