...
class Index_EweiShopV2Page extends MerchWebPage 
{
	...
	public function sync()
	{
		...
		$uniacid = $_W['uniacid'];
		$merchid = $_W['merchid'];
		$deleteSql = 'update ' . tablename('ewei_shop_goods') . ' set deleted=1 where merchid=' . $merchid . ' and uniacid=' . $uniacid;
		pdo_query($deleteSql);

		// $merchs = pdo_fetchall('select * from ' . tablename('ewei_shop_merch_user') . ' where uniacid=:uniacid and status=1 ', array(':uniacid' => $_W['uniacid']));
		// foreach ($merchs as $merch) {
			$goods = pdo_fetchall('select * from ' . tablename('ewei_shop_goods') . ' where uniacid=:uniacid and merchid=0 and status=1 and deleted=0 and checked=0 ', array(':uniacid' => $uniacid));
			foreach ($goods as $g ) {
				// 插入goods表
				$oldGoodsid = $g['id'];
				$g['id'] = 0;
				$g['merchid'] = $merchid;
				$g['catch_source'] = 'batch_copy';
				pdo_insert('ewei_shop_goods', $g);
				$goodsid = pdo_insertid();
				// 插入goods_params表
				$goods_params = pdo_fetchall('select * from ' . tablename('ewei_shop_goods_param') . ' where goodsid=:goodsid ', array(':goodsid' => $oldGoodsid));
				foreach ($goods_params as $p ) 
				{
					$p['id'] = 0;
					$p['goodsid'] = $goodsid;
				...
				// 插入goods_spec表
				$spec_item_mapping = array();
				$specs = pdo_fetchall('select * from ' . tablename('ewei_shop_goods_spec') . ' where goodsid=:goodsid ', array(':goodsid' => $oldGoodsid));
				foreach ($specs as $spec ) 
				{
					...
					$spec_itemids = array();
					$spec_items = pdo_fetchall('select * from ' . tablename('ewei_shop_goods_spec_item') . ' where specid=:specid ', array(':specid' => $oldSpecid));
					// 插入goods_spec_item表
					foreach ($spec_items as $spec_item ) 
					{
						$map_item = array();
						$map_item['old'] = $spec_item['id'];
						...
						$map_item['new'] = $spec_item['id'];
						$spec_item_mapping[] = $map_item;
					}
					pdo_update('ewei_shop_goods_spec', array('content' => serialize($spec_itemids)), array('id' => $specid));
				}

				// 插入goods_option表
				$options = pdo_fetchall('select * from ' . tablename('ewei_shop_goods_option') . ' where goodsid=:goodsid', array(':goodsid' => $oldGoodsid));
				foreach ($options as $o ) 
				{
					$ids = array();
					$arr_specs = $o['specs'];
					$arr_specs = explode("_", $arr_specs);
					if(!(empty($arr_specs))){
						$ids[] = $this->getNewItem($spec_item_mapping, $arr_specs[0]);
						$ids[] = $this->getNewItem($spec_item_mapping, $arr_specs[1]);
					}
					
					$o['id'] = 0;
					$o['specs'] = implode('_', $ids);
	...
	private function getNewItem($spec_item_mapping, $old){
		foreach ($spec_item_mapping as $spec_item){
			if($spec_item['old'] == $old){
				return $spec_item['new'];
			}
		}
		return 0;
	}
