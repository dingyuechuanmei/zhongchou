...
class Sys_EweiShopV2Page extends MerchWebPage 
{
	...
	public function sync(){
		...
		$deleteSql = 'delete from ' . tablename('ewei_shop_diypage') . ' where type=2 and merch=' . $merchid . ' and uniacid=' . $uniacid;
		pdo_query($deleteSql);

		$diyPages = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_diypage') . ' WHERE merch=0 and type=2 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
		foreach ($diyPages as $page){
			$page['id'] = 0;
			$page['merch'] = $merchid;
			pdo_insert('ewei_shop_diypage', $page);
		}
