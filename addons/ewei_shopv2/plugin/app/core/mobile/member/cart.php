...
class Cart_EweiShopV2Page extends AppMobilePage
{
	public function get_cart()
	{
		...
		$result['menus'] = $this->model->getDiypageMenus();
		$result['cartcount'] = m('goods')->getCartCount();
	...
	public function add()
	{
		...
		// 查询关注是否存在
		$is_favorite = pdo_fetchcolumn("select count(1) from ".tablename('ewei_shop_member_favorite')." where uniacid =:uniacid and goodsid=:goodsid and optionid=:optionid and openid=:openid and deleted = 0 limit 1 ",array(':uniacid'=>$_W['uniacid'],':goodsid'=>$id,':optionid'=>$optionid,':openid'=>$_W['openid']));
		if(empty($is_favorite)){
			...
			pdo_insert('ewei_shop_member_favorite',$in_data);
		}
