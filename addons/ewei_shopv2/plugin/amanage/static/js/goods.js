define(['core'], function (core) {
    ...
    modal.initClick = function () {
        ...
        $(document).on('click', '.container .fui-list .fui-list-media,.fui-list .fui-list-inner', function () {
            ...
            if (item.css('display') != 'none') {
                ...
                if (goodsid && canJump) {
                    $.router.load(core.getUrl('amanage/goods/edit', {id: goodsid}), true)
                }
            }
        });
        ...
        $(document).on('click', '.btn-goods', function () {
            ...
            if (action == 'delete' || action == 'status' || action == 'restore') {
                FoxUI.confirm(confirm_text, function () {
                    modal.stop = true;
                    core.json("amanage/goods/" + action, obj, function (json) {
                        if (json.status == 1) {
    ...
    modal.getList = function () {
        ...
        core.json('amanage/goods/getlist', obj, function (json) {
            if (json.status != 1) {
                return
            }
