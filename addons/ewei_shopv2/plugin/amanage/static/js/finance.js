define(['core'], function (core) {
    ...
    modal.initClick = function () {
        $("#btn-apply a").unbind('click').click(function () {
            $.router.load(core.getUrl('amanage/finance/apply_withdrow'), true);
        });
        ...
        $(document).on('click', '.btn-refund', function () {
            ...
            FoxUI.confirm(confirm_text, function () {
                modal.stop = true;
                core.json("amanage/finance/refund", obj, function (json) {
                    if (json.status == 1) {
        ...
        $(document).on('click', '.btn-apply', function () {
            ...
            if (type == 'alipay' || type == 'wechat' || type == 'manual' || type == 'refuse') {
                FoxUI.confirm(confirm_text, function () {
                    modal.stop = true;
                    core.json("amanage/finance/" + type, obj, function (json) {
                        if (json.status == 1) {
                            if (type == 'refuse') {
    ...
    modal.getList = function () {
        ...
        core.json('amanage/finance/getlist', obj, function (json) {
            console.log(json);
            if (json.status != 1) {
                return
            }
            ...
            if(modal.type == 0 && result.list.length > 0){
                $("#btn-apply").show();
            }
