define(['core'], function (core) {
    ...
    modal.initRemark = function (params) {
        ...
        $(".submit-params").unbind('click').click(function () {
            ...
            FoxUI.confirm("确定保存修改吗？", function () {
                modal.stop = true;
                core.json("amanage/order/op/remarksaler", obj, function (json) {
                    modal.stop = false;
                    if (json.status != 1) {
    ...
    modal.initAddress = function (params) {
        ...
        $(".submit-params").unbind('click').click(function () {
            ...
            FoxUI.confirm("确定要修改收货信息吗？", function () {
                modal.stop = true;
                core.json("amanage/order/op/changeaddress", obj, function (json) {
                    if (json.status != 1) {
                        FoxUI.toast.show(json.result.message);
                        modal.stop = false
                    } else {
    ...
    modal.initSend = function (params) {
        ...
        $(".submit-params").unbind('click').click(function () {
            ...
            FoxUI.confirm(confirm_text, function () {
                modal.stop = true;
                core.json("amanage/order/op/" + route, obj, function (json) {
                    if (json.status != 1) {
                        FoxUI.toast.show(json.result.message)
                    } else {
    ...
    modal.initCancel = function (params) {
        ...
        $(".submit-params").unbind('click').click(function () {
            ...
            FoxUI.confirm(confirm_text, function () {
                modal.stop = true;
                core.json("amanage/order/op/sendcancel", obj, function (json) {
                    if (json.status != 1) {
                        FoxUI.toast.show(json.result.message);
                        modal.stop = false
                    } else {
    ...
    modal.initRefund = function (params) {
        ...
        $(".submit-params").unbind('click').click(function () {
            ...
            FoxUI.confirm("确认提交吗？", function () {
                modal.stop = true;
                core.json("amanage/order/op/refund", obj, function (json) {
                    if (json.status != 1) {
                        FoxUI.toast.show(json.result.message);
                        modal.stop = false
                    } else {
