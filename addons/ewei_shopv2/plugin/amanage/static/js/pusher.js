define(['core'], function (core) {
    ...
    modal.initClick = function () {
        ...
        $(document).on('click', '.container .fui-list .fui-list-media,.fui-list .fui-list-inner', function () {
            ...
            if (item.css('display') != 'none') {
                ...
                if (goodsid && canJump) {
                    modal.jumped = true;
                    $.router.load(core.getUrl('amanage/pusher/edit', {id: goodsid}), true)
                }
            }
        });
        ...
        $(document).on('click', '.btn-goods', function () {
            ...
            if (action == 'edit') {
                ...
                if (status == 1) {
                    var confirm_text = "确认显示吗？";
                    obj.status = 1
                } else {
                    var confirm_text = "确认隐藏吗？";
                    obj.status = 0
                }
            }
            if (action == 'delete' || action == 'status') {
                FoxUI.confirm(confirm_text, function () {
                    modal.stop = true;
                    core.json("amanage/pusher/" + action, obj, function (json) {
                        if (json.status == 1) {
        ...
        $(".btn-goods-batch").unbind('click').click(function () {
            ...
            if (action == 'delete') {
                ...
                var status = _this.data('status');
                if (status == 1) {
                    var confirm_text = "确认显示选中商品吗？";
                    obj.status = 1
                } else {
                    var confirm_text = "确认隐藏选中商品吗？";
                    obj.status = 0
                }
            }
            if (action == 'delete' || action == 'status') {
    ...
    modal.getList = function () {
        ...
        // console.log(obj);
        core.json('amanage/pusher/getlist', obj, function (json) {
            if (json.status != 1) {
                return
            }
