define(['core', 'tpl'], function (core, tpl) {
    ...
    modal.loginPost = function (obj) {
        ...
        core.json('amanage/login', obj, function (json) {
            setTimeout(function () {
                if (json.status == 1) {
                    if (json.result.backurl) {
                        location.href = json.result.backurl
                    } else {
                        location.href = core.getUrl('amanage')
                    }
                } else {
                    FoxUI.loader.hide();
    ...
    modal.initHome = function () {
        core.json('amanage/get_today', {}, function (json) {
            if (json.status == 1) {
                $("#today_count").text(json.result.today_count);
                $("#today_price").text(json.result.today_price);
            }
        });
        core.json('amanage/get_order', {}, function (json) {
            if (json.status == 1) {
        ...
        core.json('amanage/get_shop', {}, function (json) {
            if (json.status == 1) {
    ...
    modal.initPerson = function () {
        $("#btn-submit").unbind('click').click(function () {
            ...
            core.json('amanage/set', obj, function (json) {
                if (json.status == 1) {
                    FoxUI.toast.show("保存成功");
                    if (json.result.changepass) {
                        location.href = core.getUrl('amanage/login');
                        return
                    }
                } else {
                    FoxUI.toast.show(json.result.message)
        ...
        $("#btn-logout").unbind('click').click(function () {
            ...
            FoxUI.confirm("当前已登录，确认要退出吗？", function () {
                core.json('amanage/login/logout', {}, function () {
                    location.href = core.getUrl('amanage')
                }, true, true)
            })
        })
    ...
    modal.initShop = function () {
        ...
        $("#btn-submit").unbind('click').click(function () {
            ...
            core.json('amanage/shop', obj, function (json) {
                if (json.status == 1) {
                    FoxUI.toast.show("保存成功")
                    location.href = core.getUrl('amanage')
                    return
