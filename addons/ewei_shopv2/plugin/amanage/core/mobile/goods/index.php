...
class Index_EweiShopV2Page extends AmanageMobilePage 
{
	...
	protected function post() 
	{
		...
		include $this->template('amanage/goods/post');
	}
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('ewei_shop_goods') . ' WHERE id in( ' . $id . ' ) AND merchid='.$_W['merchid'].' AND uniacid=' . $_W['uniacid']);
	...
	public function raisePusherAdd()
    {
        global $_W;
        global $_GPC;
        if ($_W['ispost']){
        ...
        include $this->template('amanage/goods/raise');
