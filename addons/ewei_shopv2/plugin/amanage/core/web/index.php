	...
	public function setting() 
	{
		...
		if($_W['ispost']){
			...
			if(!empty($arr['keyword'])){
				...
				$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1' , array(':uniacid'=>$_W['uniacid'],':module'=>'ewei_shopv2',':name'=>'ewei_shopv2:amanage'));
				if (!empty($rule)) {
					...
					$rule_data['name'] = 'ewei_shopv2:amanage';
			...
			m('common')->updatePluginset(array('amanage' => $arr));
			plog('amanage.setting.save', '保存基本设置');
			show_json(1);
		}
		$qrcode = m('qrcode')->createQrcode(mobileUrl('amanage', array(), true));
		include $this->template();
	}
