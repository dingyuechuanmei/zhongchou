...
class PosteraProcessor extends PluginProcessor 
{
	...
	{
	    global $_W;
	    $share_log = pdo_fetch("SELECT id FROM ".tablename('ewei_shop_share_log')." WHERE uniacid=:uniacid AND mid=:mid AND sid=:sid",array(':uniacid'=>$_W['uniacid'],':mid'=>$member['id'],':sid'=>$qrmember['id']));
	    if(empty($share_log)){
	        pdo_insert('ewei_shop_share_log',array(
	            'uniacid'=>$_W['uniacid'],
	            'mid'=>$member['id'],
	            'sid'=>$qrmember['id'],
	            'share_type'=>0,
			...
			if (!(empty($cset))) 
			{
				if ($member['isagent'] != 1) 
				{
					if (($qrmember['isagent'] == 1) && ($qrmember['status'] == 1)) 
					{
						...
						{
							if(!empty($cset['order_agent'])){
						        $judeOption =  empty($member['agentid']) || empty($member['agentid_status']);
						    }else{
						        $judeOption = empty($member['agentid']);
						    }
							if ($judeOption) 
							{
